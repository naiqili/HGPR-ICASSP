function [varargout] = sp_gp(hyp, inf, mean, cov, lik, x, y, xs, ys)
...
if size(y,2)>1      % deal with (independent) multivariate output y by recursing
  ...
  for i=1:d
    ...
    if i==1, [varargout{:}] = sp_gp(in{:});    % perform inference for dimension ..
    else           [out{:}] = sp_gp(in{:});             % .. number i in the output
      if nargin==7, no = 2;
...
try                                                  % call the inference method
  ...
  if nargin>7   % compute marginal likelihood and its derivatives only if needed
    ...
    if nargout<=1
      ...
      dnlZ.lik = 0; dnlZ.cov = zeros(size(dnlZ.cov));
    end
  end
catch
  msgstr = lasterr;
